...
with T4:
    ...
    if not enable_date_filter : # all Data
        filter = ((album_listening_df['played_at_date'] >= min_date) & (album_listening_df['played_at_date'] <= max_date))
        data = album_listening_df
        start_dt = data['played_at_date'].min()
        day_diff = (data['played_at_date'].max() - data['played_at_date'].min()).days
           
    
    elif enable_date_filter and start_dt and not end_dt: # only the start date
        filter = album_listening_df['played_at_date'] == start_dt 
        data = album_listening_df[filter]
        day_diff = 1
        

    else:
        filter = ((album_listening_df['played_at_date'] >= start_dt) & (album_listening_df['played_at_date'] <= end_dt))
        data = album_listening_df[filter]
        day_diff = (end_dt - start_dt).days
        
    
    T4_col1, T4_col2 = st.columns([2, 1])
    ...
    with T4_col1:
        with st.container(border=True):
            ...
            if day_diff > 6:
                ...
                y="Total Listening Time (Minute)",
                line_shape="spline",
                title="Weekly Album Listening Trend"
                )
                ...
                st.plotly_chart(fig, width='stretch', theme="streamlit", key = "listening stats line Graph albums")
            ...
            if day_diff <= 6:
                ...
                st.plotly_chart(fig, width = "stretch", theme="streamlit", key="listening_stats_bar_chart_albums")
